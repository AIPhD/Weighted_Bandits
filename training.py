...
theta_source = target.theta_opt + c.kappa * np.random.multivariate_normal(mean=np.zeros(c.dimension), 
...
def ucb_weighted(x, A, alpha_1, alpha_2, theta_s, theta_t, gamma):
    ...
    expl = gamma * np.sqrt(np.diag(np.matmul(x, np.matmul(np.linalg.inv(A), x.T))))      #TO OPTIMIZE
...
def weighted_training(target=target, alpha_1=c.alpha):
    ...
    for i in range(0, c.epochs):
        ...
        r_estim_1 = np.dot(theta_source, instance)
        r_estim_2 = np.dot(theta_estim, instance)
        noise = c.epsilon * np.random.normal()
        r_real = np.dot(theta_target, instance) + noise
    ...
    # plt.plot(np.cumsum(regret_evol))
    #plt.show()
    return regret_evol
...
def compared_alphas():
    ...
    alphas = [0, 0.1, 0.2, 0.5, 0.7, 0.99]
    regrets = []

    for alpha in alphas:
        regrets.append(weighted_training(target=target, alpha_1=alpha))

    return regrets, alphas
